...
**This unit is the alpha hardware, and it undercuts §9's premise.** The dump is from a colleague's trainer, and the team's colleagues ride **KICKR CORE v2** — a different machine from the **KICKR v2 (2016)** this section was written about. Two similar names, two products: the 2016 v2 is the old flagship that predates FTMS; the CORE v2 is a current Core, and the dump shows it speaking FTMS with ERG and cadence.
...
- **Confirmed on hardware** (Polar H10, 55 packets, 2026-08-29, #11): flags alternate `0x10` (53 packets) and `0x00` (2) — the strap sends RR only when a beat fell in the notification window. **Six packets carried two intervals**, so the variable count is real at *resting* heart rate, not just under load; reading only the first would have dropped those beats while still displaying a correct bpm, which is a failure that never shows on screen. Bytes pinned as a regression test in `web/src/lib/ble/heartrate.test.ts`. Energy expended (bit 3) was never set by this strap, so that skip stays unit-tested only. Note the H10's HR field is smoothed: it read 62 bpm while the instantaneous RR implied ~55 — expected, not a parse error.
...
*Re-read against the primary pages on 2026-09-08 (#229). The findings below held; two dates were missing and are now the last two bullets. WATTROOM.md's row was refreshed at the same time and no longer cites Nov-2024.*

**The Nov-2024 agreement WATTROOM.md used to cite is no longer current.** A new [API Agreement](https://www.strava.com/legal/api) + [API Policy](https://www.strava.com/legal/api_policy) took effect **June 1, 2026**. What changed and what it means:
...
- **Obligations**: raw Strava payloads are a **≤7-day cache** (§6.2); derived values inherit the rules — everything wiped **≤30 days after deauthorization** (webhook `athlete`/`authorized:false`; new `/oauth/revoke`); on-Strava activity deletions reflected ≤48 h. Clean fit with our seams: durable per-day load numbers in Postgres, raw payloads treated as re-fetchable cache. *Deauthorization is handled (#2823): Strava's push webhook is the primary signal and an upload refused for a revoked grant the backstop; either way Strava is asked to refresh the grant first, since its events are unsigned, and only a refusal naming the refresh token forgets the tokens and activity ids.*
- **A hard date nobody has diarised: 2026-09-30.** The subscription requirement bit on June 30, 2026, but active developers without one got **three months free** — and that window closes at the end of September 2026. After it, an unsubscribed developer account loses Standard Tier, which is what the *existing* auto-upload runs on. ([developer program update](https://communityhub.strava.com/insider-journal-9/an-update-to-our-developer-program-13428))
- **A second date, further out: 2027-06-01 technical migration.** Three breaking changes. The base URL moves from `https://www.strava.com/api/v3` to **`https://api-v3.strava.com`** — no `www`, and per the [changelog](https://developers.strava.com/docs/changelog/) it does not exist to test against until **2027-01-04**. **Authorization tokens must travel in headers rather than form or query params** on API requests; our upload path already sends `Authorization: Bearer` (strava.go), so that half is done. And **`oauth/deauthorize` retires in favour of `oauth/revoke`** — done in #1093.
  **Do not read the header rule as applying to `oauth/revoke`.** It does not: revoke is RFC 7009 in shape, the *client* authenticates with Basic auth and the token being revoked is a form field, which is what Strava's own curl example shows. The rule is about requests that authenticate *with* a token, not about the one request whose subject *is* a token. Conflating the two is the mistake this bullet made when it was written.
