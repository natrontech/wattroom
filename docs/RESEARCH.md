...
- **A second date, further out: 2026-06-01 → 2027-06-01 technical migration.** Three changes, all breaking, all announced in the same update: the base URL moves from `https://www.strava.com/api/v3` to **`https://www.api-v3.strava.com`**; **authorization tokens must travel in headers, not form or query params**; and **`oauth/deauthorize` retires in favour of `oauth/revoke`**. Our upload path already sends `Authorization: Bearer` (strava.go), but `RevokeGrant` still puts the token in a query string against the retiring endpoint — so this is a real code change with a deadline, not a note. Tracked separately.
