...
**The public issue names nobody** (amended 2026-09-06, #737). The repository's issues are world-readable, and a report's body used to open with the rider's display name and the room they were in. Neither is needed to fix anything: the JSONL record on disk keeps both for triage, the issue URL goes back to the rider who filed it, and the fingerprint keeps the full route so per-room deduplication is unchanged. The body and the title carry the screen (`/r/…/sessions`), not the room. This ADR had said nothing about who may read a report; it does now.
