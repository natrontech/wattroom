...
| [0030](0030-what-wattroom-emails.md) | Two classes of mail: an unconditional alarm and one opt-in ride switch | accepted | Builds on [0029](0029-one-account-a-set-of-credentials.md). Extends the WATTROOM.md "Privacy" row to the inbox; the palette copy it accepts is the one `httpx.WritePage` already made |
| [0031](0031-the-server-unfurls-links-riders-paste.md) | The server unfurls links riders paste, behind a pinned-address fetch | accepted | Builds on [0009](0009-login-gated-app.md). Replaces the browser-only oEmbed note in `LinkPreview.svelte`. Read with [0032](0032-a-gif-picker-proxied-through-the-server.md): bytes from a host the rider chose load directly, bytes from a host another member chose go through us |
| [0032](0032-a-gif-picker-proxied-through-the-server.md) | The GIF picker is proxied, and it posts a plain chat message | accepted, amended 2026-09-06 (#909: GIPHY, not Tenor — that API shut down; the quota is server-wide) | Answers the picker #279 parked. Stickers stay open ([#879](https://github.com/natrontech/wattroom/issues/879)) |
...
| [0034](0034-a-session-leaves-one-recap.md) | A session leaves one recap: who was here, and for how long | accepted | Amends [0022](0022-room-events-are-ephemeral.md) on the terms it set — events stay ephemeral, one artifact per session does not. Answers [#985](https://github.com/natrontech/wattroom/issues/985); keeps WATTROOM.md's metrics and rides rules untouched |
...
| [0037](0037-a-desktop-shell-for-what-the-browser-cannot-reach.md) | A desktop shell for what the browser cannot reach; the web app stays the product | accepted | Amends [0004](0004-chrome-first-with-native-escape-hatch.md) on its own terms. Version namespace is deliberately not [0019](0019-tagged-releases-and-a-self-converging-vm.md)'s CalVer; WATTROOM.md's "iOS: web-only, forever" is untouched. Answers [#296](https://github.com/natrontech/wattroom/issues/296) |
