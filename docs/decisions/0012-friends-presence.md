...
**What "close the app" rests on (#1506, #1740, #2087).** Presence derived from a live socket is only honest while the server can tell the socket apart from a dead one, and a laptop that sleeps, a NAT that drops or a phone that loses signal closes nothing the server ever hears. So both sockets — the lobby's and each room's — ping their peer every **5 s** (every 30 s until #2131) and are dropped when the pong does not come back within **5 s** (`server/internal/hub/keepalive.go`); the rider then reads offline, leaves the roster, and gets their socket budget back. This is the mechanism, not a new decision: no timestamp is stored, nothing is persisted, and the boolean a friend sees is unchanged. The alternative — last-seen timestamps — stays rejected: it would record presence, which is exactly what this ADR says the server does not do.
