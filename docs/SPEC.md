...
| **Crew**              | The layer above rooms ([ADR-0038](decisions/0038-the-crew-is-the-layer-above-rooms.md)): a name, an icon, its rooms and the people in them. A crew has **two doors**, and both admit to the crew rather than to one room ([ADR-0038](decisions/0038-the-crew-is-the-layer-above-rooms.md) amended, #1236 and #2245). Its code or share link (`/c/{code}`) is the invite a member hands out. A room listed in the public directory ([ADR-0039](decisions/0039-the-public-room-directory.md) amended, #2245) is the other: joining it joins the crew first and the room second, so the joiner also gets the crew's other open rooms, its code, and its people. Once in: its open rooms you walk into, its private ones admit their members and whoever was let in. Rooms are channels and have no codes of their own. The owner or an admin can make a new code (#1930); the old one, and every link carrying it, stops working at once. A crew carries no voice, deck, session, game or metrics; everything live stays the room's. Every rider founds one crew, named after them, made with their first room — and may come to own more when one is handed to them (#1208); "your own crew" is the one you founded (#1928); the sidebar shows one crew at a time ([ADR-0020](decisions/0020-the-app-takes-discords-shape.md), amended), opening in the **main crew** a rider in more than one has named (#2144) — the account's choice, on every device.                               |
...
| **Block**             | One stretch of the flattened timeline the rider rides — "block 3 of 6" on the riding screen, the header's target, the strip's next-up. Repeats expand into blocks; the room's expansion budget is counted in blocks (#1713).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **Jukebox**           | The room's one music surface (ADR-0018): a shared YouTube queue on a server-owned playhead. **Deck** = what is playing, **up next** = the queue, **just played** = the last 5, kept in the tick and, since #1432, in `track_plays` — a room the server has just brought up shows what the log remembers, videos and library tracks alike, until it plays something of its own.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **Room playlist**     | A saved, named, ordered list of jukebox entries — videos, pasted playlists, library tracks ([ADR-0045](decisions/0045-a-saved-playlist-is-a-saved-queue.md)) — that belongs to a room (#627) — survives past any one queue. A room can save several; one is marked **active**, which is what **autoplay** and the panel's default "queue" button use. A member creates one, adds to one and **reorders** one; renaming, deleting, removing a track and making one active are the coach's and the owner's (#695). The split is that a member may put things on the room's shelf and arrange them, and may not take anything off it — reordering is the queue's own move applied to the shelf (#1428), and it changes no more about what autoplay plays than a member's add already does. Queueing a room playlist into the live queue is a straight append of its entries, subject to the same caps the queue always had (`maxQueue`, `maxQueuedTracks`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| **Announcement**      | One line a coach marked, standing at the top of a room's **Board**, and on its Lounge and Chat ([ADR-0057](decisions/0057-an-announcement-is-a-chat-message-marked.md), #2408): no session Thursday, the route changed. It **is a chat message**, marked from that message's own menu — there is no announcement composer, and the notice keeps the message's author and time. **One per room**: marking another replaces it. Marking and clearing are the **coach's and the owner's**. The marked line is kept past the 500-message cap. Drawn on the Lounge only while the room is idle, and always above Chat's log. |
| **Board**             | The first place in a room ([#2413](https://github.com/natrontech/wattroom/issues/2413)), above the Lounge: what the room and its crew wrote down — the **announcement** at the top, then the crew's **pins**. Its position is the feature rather than a preference: a notice on the fifth row is filed, one on the first is the door a rider comes through. `/r/[slug]/board`; the earlier `/r/[slug]/pins` redirects. |
| **Pin**               | A title and a block of lines on a crew's board ([ADR-0056](decisions/0056-a-crew-pins-what-it-keeps-needing.md), #2405): the handful of facts a crew keeps needing and nothing else holds — a game server and its password, the Discord link, the door code. Owned by the **crew**, so every room of it shows the same board, and read and written in a room's **Board** place. The body is plain text with one rule: a line written `Label: value` draws as a row with its own copy button, everything else stays prose. **Everyone in the crew** pins, edits and unpins; nothing records who wrote one. Bounded at **20 pins** per crew, a **40-character** title and a **1000-character** body (`protocol`, generated both sides). A pin is not a secret: every member reads it, and the editor says so. |
...
| **Planned session**   | A session put on a room's calendar for a time (#116). Members **RSVP**: in, or out — there is no maybe. Three states are _stored_ and only two are ever asked for: **in**, **out**, and the room not having heard from you, which is the absence of an answer rather than a third thing a rider can say (#1011). It is not a second kind of object, and it is not a _room event_, which is the chat line above.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Streak**            | Consecutive **weeks**, counted from Monday-start weeks, in which something was ridden. The current week is forgiving: a streak survives until that week ends without a ride, so a crew that always rides on Saturday does not read as broken on Tuesday. There are **two** streaks and they are different numbers — a **room streak** and a **rider streak**, below. Say which one you mean; unqualified "streak" is ambiguous.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Room streak**       | Consecutive weeks in which a **room** held at least one session, in **UTC** weeks — a room's riders are in several zones and a room has none of its own. It is a consistency number and it is the one on screen: the Lounge and the Sessions place both label it **this room's streak**. It pays **no** XP — a room streak that paid would let a rider join a room on a six-week run and be paid for other people's rides.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Rider streak**      | Consecutive weeks in which a **rider** rode at least once — in this room, in another, or solo — in **their own** weeks (the day boundary under Stats formulas). This is the streak that **pays**: the XP streak bonus below is `25 ×` the rider's own current-week streak, capped at 250. It is read before the ride being saved lands, so the ride extends the streak from the next ride on. Never displayed as the room's number.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| **Consistency**       | Showing up, as opposed to how hard you rode — the thing a room's own numbers are about ([RESEARCH.md §14.7](RESEARCH.md)). A room expresses it two ways: its **room streak**, and its **sessions this month** against its own last month. It is never a per-rider score and never a ranking; a rider sees only their own turnout ([ADR-0036](decisions/0036-what-a-room-shows-about-its-members.md)).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| **Weekly board**      | The one ordered surface a room may have ([ADR-0036](decisions/0036-what-a-room-shows-about-its-members.md)): each member's **kJ** and time ridden here **this week**, bracketed by **Category**, resetting Monday with the room streak's week. **Off until the room's owner turns it on**, and never the room's front page. It is the only room surface that publishes a number derived from one member's rides, so it is disclosed twice — the **join door** says a room keeps one before anyone walks in (#1651), and each member has their own **include me** switch (`memberships.on_board`, default on). A rider is on it only while they are a member: leaving takes their row with them. |
...
| Put up and take down the room's announcement ([ADR-0057](decisions/0057-an-announcement-is-a-chat-message-marked.md)) | ✓     | ✓     | –                           | ✓            |
...
- A new room is **open to its crew**; rooms that existed at the cutover stayed
  ...
  same ladder that lists it publicly, one step further. That top step is a
  **crew-level** act, not a room-level one (ADR-0038 amended, #2245): listing
  a room opens the crew to every signed-in rider, so it also opens the crew's
  other open rooms, which belong to other owners. A room shut to its crew
  leaves the directory with it — `listed ⇒ crew_visible` is enforced in SQL,
  not only by the ladder.
...
Session lifecycle: room idles (voice/jukebox lounge) → coach picks workout → 10 s countdown **(default)** → shared timeline runs → riders execute their own %FTP targets → session closes when the timeline ends (or coach ends it) → server computes stats + medals in one transaction. Late joiners sync to the current timeline position. A member stopping mid-session pauses _their own_ targets (auto-pause) — the shared timeline never waits. The picked workout is then ridden as planned: **nobody skips a block or adds a minute in a room** ([ADR-0046](decisions/0046-one-riding-surface.md) amended, #1635). Pause, resume and end are the coach's only holds on the shared timeline — the coach cannot add a minute, and ending the session is the only escape; changing the work itself is the workout editor's job. A rider **alone** owns their clock outright and keeps `+1 min` and `Skip block`.
...
- **The day boundary is the rider's own** (#2063). Every per-rider calendar
  ...
  - **Room buckets are UTC**, and that is a different question, not an
    oversight: a room's riders sit in several zones and a room has no zone of
    its own. The **room streak**, its month, its session days and its week
...
- **XP**: `1 kJ = 1 XP`, plus per-ride bonus `execution% × 50`, plus streak bonus `25 × current-week-streak` (capped at 250) — the **rider streak**, their own weeks wherever they rode, never the room streak the Lounge displays (Glossary). Level thresholds: level n requires `500 × n^1.6` cumulative XP (a winter of 3 rides/week ≈ level 25–30).
...
- **Who is in is named; who is out is a number** (#1011). A plan reads **`4 in · 2 out · 9 unanswered`**, with the parts that are nobody left out, and the names of the riders who are in beside it. The count is what changes a planner's decision — hold the session, or move it — and the list of who declined would only add pressure to a decision a rider has already made. Rooms are small, so naming is close to automatic once it is shown at all; this is the one place the room deliberately says less than it knows.
- **A decline is cleared when the session moves to a new time**, on exactly the condition that re-arms the hour-before reminder: the time really changed. The session that was turned down is not the session now planned, so the people who turned it down are asked again. An "in" survives a move — dropping it would empty a line the room reads, while a decline that outlived a move would silence a reminder for a session the rider never turned down.
...
- **A room nobody is in is forgotten after 2 h** ([#2297](https://github.com/natrontech/wattroom/issues/2297)). The live room — its tick goroutine, jukebox queue, chat buffer, timeline, roster and per-rider sample accumulators — is built by the first join and, until this, was held until the process was replaced. Two hours of **no open sockets, nobody in the voice channel, and a session that is idle or done** releases it; the next join rebuilds the room from nothing ([ADR-0052](decisions/0052-a-room-ride-the-server-loses-comes-back-from-the-browser.md)'s re-form path, the same one a restart takes). What goes is what was never durable: the **queue and what it was playing**, the room's "just played", the ephemeral **timeline**, who the room had **seen**, and any sounding clip. Chat comes back from the database on the next join, and a ride was saved when its session closed. Two hours is deliberately long rather than tuned — nobody returns to "the room I was just in" after that, so the reclaim is one a rider cannot notice. A session in **countdown, running or paused** does not release the room however empty it is: those still hold samples nobody has saved, and the room's own clock is what closes and saves them.
...
- What reaches a rider who is **not looking** — the tab hidden, or the window not the front one: a chat message, a rider arriving, a **session** starting, a poke. A focused, visible room speaks for itself and gets a toast instead.
...
- **Offered once, in context** (#1485): the first time a rider sees a **planned session** in one of their rooms — Home's _What's next_ — an inline line offers to turn them on, alongside the plan that makes them worth having. It renders only where the button can succeed: notifications possible on this device, not already on, not blocked by this browser, and not switched off or waved away before. **No browser permission dialog until the rider presses the button** — the 95 % rule rules out a prompt in the first minute. Turning them on, or waving the offer away, retires it on that device for good; Settings → Notifications is where the answer is changed afterwards.
