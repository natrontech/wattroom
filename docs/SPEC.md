...
| **Deleted rides**       | An offsetting row when a rider deletes a ride: **amount = that ride's XP**, `ref` = the ride's id. The delete is a hard delete, so the ride leaves `sum(rides.xp)`; this puts the same number back and `user_total_xp` does not move. Not a source of new XP and never an achievement's measure — deleting a ride is privacy, not work ([ADR-0047](decisions/0047-deleting-a-ride-keeps-its-xp.md)).                                                                                                                                                                                                                                                                                                                                               |
