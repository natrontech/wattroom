...
| **Announcement**      | One line a coach marked, standing at the top of a room's Lounge and Chat ([ADR-0057](decisions/0057-an-announcement-is-a-chat-message-marked.md), #2408): no session Thursday, the route changed. It **is a chat message**, marked from that message's own menu — there is no announcement composer, and the notice keeps the message's author and time. **One per room**: marking another replaces it. Marking and clearing are the **coach's and the owner's**. The marked line is kept past the 500-message cap. Drawn on the Lounge only while the room is idle, and always above Chat's log. |
| **Pin**               | A title and a block of lines on a crew's board ([ADR-0056](decisions/0056-a-crew-pins-what-it-keeps-needing.md), #2405): the handful of facts a crew keeps needing and nothing else holds — a game server and its password, the Discord link, the door code. Owned by the **crew**, so every room of it shows the same board, and read and written in a room's **Pins** place. The body is plain text with one rule: a line written `Label: value` draws as a row with its own copy button, everything else stays prose. **Everyone in the crew** pins, edits and unpins; nothing records who wrote one. Bounded at **20 pins** per crew, a **40-character** title and a **1000-character** body (`protocol`, generated both sides). A pin is not a secret: every member reads it, and the editor says so. |
