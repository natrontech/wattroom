...
Above rooms sits the **crew** ([ADR-0038](decisions/0038-the-crew-is-the-layer-above-rooms.md), as amended): `crews` (name, icon, picture, owner, a six-character join code), `crew_roles` (member / admin / banned — membership is a row, the owner holds none), `rooms.crew_id` (permanent), `rooms.crew_visible` (open to the crew, or private) and `room_grants` (a private room's named exceptions). A crew carries nothing live — no voice, deck, session or metrics — so the hub never sees it. Who may enter which room is answered in exactly one place, the `visible_rooms` view, and every gate and visibility join reads it rather than re-deriving it: a join that forgets it fails to compile instead of silently over-permitting. The crew's code is the only invite; `/r/{slug}` is a room's address, not a door.
