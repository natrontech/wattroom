...
for (const line of execFileSync('sh', [script, 'print'], {
	...
	if (match) derived[match[1]] = match[2];
}
...
// started.
function port(name) {
	const value = Number(derived[name]);
	if (!Number.isInteger(value) || value <= 0) {
		throw new Error(
			`${script} print gave no usable ${name} (got ${JSON.stringify(derived[name])})`,
		);
	}
	return value;
}

export const WEB_PORT = port('WATTROOM_E2E_WEB_PORT');
export const API_PORT = port('WATTROOM_E2E_API_PORT');
...
export const DB_DSN = process.env.WATTROOM_DB ?? derived.WATTROOM_DEV_DSN;
