 ...
 * glyph on the tile a SECOND rider's voice channel draws, and a sentence in
 * that rider's log. Two riders because the timeline drops your own presence lines
...
test('a rider picks a state, and the voice channel is told which', async ({
	riders,
	channels,
}) => {
	test.skip(
	...
	// A desk-sized window: the you-panel's Away row and the channel's tiles
	// are only on screen together from `xl`.
	...
	// What the channel was told — the hub's own line, through B's socket.
