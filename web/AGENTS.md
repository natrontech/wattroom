...
- SPA constraints: `ssr = false` in `routes/(app)`, where the app lives; `routes/(site)` is the prerendered public pages ([ADR-0061](../docs/decisions/0061-the-public-pages-are-prerendered.md)), which must render without a browser. No server-only SvelteKit features (form actions, +page.server.ts) — the Go server is the only backend.
