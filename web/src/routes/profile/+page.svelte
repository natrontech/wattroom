<script lang="ts">
	...
	const signedIn = $derived(!!account.me);
</script>
...
<main class="mx-auto max-w-2xl px-6 py-10">
	...
	{:else if !signedIn && account.providers.length > 0}
		<div
	...
	{#if account.loaded && (signedIn || account.providers.length === 0)}
		{#if !signedIn}
		...
		{#if signedIn}
			...
			<section class="border-muted/15 mt-3 rounded-lg border p-6">
				...
				<ul class="text-muted mt-3 space-y-1.5 text-xs">
					<li>
						Rides are private by default — sharing is per ride, and opt-in.
					</li>
					<li>
						Live power is visible only inside a room, only while you're riding
						it.
					</li>
					<li>
				...
				{#if confirmDelete}
					...
					<div class="border-z6/50 bg-z6/10 mt-4 rounded-lg border p-5">
						...
						<p class="text-muted mt-1.5 text-xs leading-relaxed">
							...
							and memberships. There is no undo and no backup we can restore
							from — that's the point of a full purge.
						</p>
