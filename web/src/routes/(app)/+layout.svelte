<script lang="ts">
	...
	// '/' is the prerendered landing outside this shell (ADR-0061); /enter,
	// where a signed-in '/' goes, sends a stale session back to it itself.
	const publicPath = $derived(
		page.url.pathname.startsWith('/login') ||
			page.url.pathname === '/enter' ||
	...
	const framed = $derived(
		!!account.me &&
			!page.url.pathname.startsWith('/login') &&
			// Signed in, /enter is the hold the routing effect leaves from.
			page.url.pathname !== '/enter' &&
	...
	// The OAuth round-trip lands on "/", which the server sends on to /enter
	// (ADR-0061) — pick up the stashed deep link, and with no stash, a
	// signed-in /enter goes Home (#126), or to the door of the
	...
	$effect(() => {
		...
		} else if (page.url.pathname === '/enter') {
			// Open in the crew the sidebar would (#2144, #2576) — which needs
			// the crew list, and /enter is outside the frame that starts it. Read
