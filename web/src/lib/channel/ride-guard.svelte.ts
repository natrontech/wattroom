 ...
 * summary is lost. So while this tab rides a session — live, on a trainer —
 * that one navigation asks first, as /ride and /ramp's leave guard does
...
export function guardTheRide(): void {
	...
	beforeNavigate((navigation) => {
		...
		if (!isLivePhase(here.live.tick?.state.phase) || !here.ride.trainer) return;
