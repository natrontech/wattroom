/**
 * What the dock's player knows that the rest of the channel needs (#216,
 * #286): the iframe lives in JukeboxDock, the transport and the sync readout
 * live wherever the channel's UI is.
 */
export const playerInfo = $state({
	/** Seconds, for the seek bar. 0 until the player reports one. */
	duration: 0,
	/** A livestream has no seekable timeline at all — the deck rides the edge. */
	live: false,
	/** Signed seconds this rider is ahead (+) or behind (−) the deck. */
	drift: 0,
...
/** How far out of step is still "in sync" for riders listening together. */
