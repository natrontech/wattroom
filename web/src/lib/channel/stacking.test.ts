...
describe('channel stacking (#483)', () => {
	...
	const floatingPlayer = layer('lib/channel/JukeboxDock.svelte', /: 'z-(\d+)'/);
	const poppedStage = layer(
		'lib/channel/Stage.svelte',
		/fixed top-24 left-24 z-\[(\d+)\]/,
	);
	// Both moved out of ChannelShell with their seams (#686). The layers are the
	...
	const chatSheet = layer(
		'lib/channel/PeopleSheet.svelte',
