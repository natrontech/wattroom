...
/**
 * A theme's tokens as CSS declarations. Rendering a theme is nothing but
 * writing these somewhere they inherit from — `:root` and `.cave` for the app,
 * a `style` attribute for a container that shows a theme it has not adopted
 * (the picker's swatches, /dev/themes). One spelling, so a token added to
