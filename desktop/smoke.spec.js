...
test("the window opens and the bridge carries what the app looks for", async () => {
	...
	// Unpackaged, nothing is ever downloaded (#1303): the app must hear
	// "nothing waiting", not an IPC error.
	expect(await win.evaluate(() => window.wattroom.updateReady())).toBeNull();
