...
func TestAmendRideGrowsASavedRideOnlyForward(t *testing.T) {
	...
	voice := testx.Voice(t, st, testx.Crew(t, st, "Amend", user.ID), "Amend", false)
	...
	if err := saver.save(ctx, voice, "", "W", workoutJSON, startedAt, []hub.RiderRecord{{Rider: rider, Samples: samples(70)}}); err != nil {
		t.Fatal(err)
	}
	...
	saver.AmendRide(ctx, voice, "", "W", workoutJSON, startedAt, hub.RiderRecord{Rider: rider, Samples: samples(130)})
	...
	saver.AmendRide(ctx, voice, "", "W", workoutJSON, startedAt, hub.RiderRecord{Rider: rider, Samples: samples(100)})
	...
	saver.AmendRide(ctx, voice, "", "W", workoutJSON, startedAt.Add(time.Minute), hub.RiderRecord{Rider: rider, Samples: samples(200)})
	var rides int
...
func TestAnAmendedRideDoesNotPayItsOwnStreak(t *testing.T) {
	...
	t.Cleanup(func() { _, _ = st.Pool.Exec(context.Background(), "delete from users where id = $1", user.ID) })
	voice := testx.Voice(t, st, testx.Crew(t, st, "Streak", user.ID), "Streak", false)
	...
	saver.AmendRide(ctx, voice, "", "W", workoutJSON, startedAt, hub.RiderRecord{Rider: rider, Samples: samples(140)})
	amended := xpOf()
	if amended <= saved {
	...
	if err := saver.save(ctx, voice, "", "W", workoutJSON, startedAt, []hub.RiderRecord{{Rider: rider, Samples: samples(140)}}); err != nil {
		t.Fatal(err)
	}
...
func TestAnAmendedRideIsJudgedOnTheWholeRide(t *testing.T) {
	...
	t.Cleanup(func() { _, _ = st.Pool.Exec(context.Background(), "delete from users where id = $1", user.ID) })
	voice := testx.Voice(t, st, testx.Crew(t, st, "Rejudge", user.ID), "Rejudge", false)
...
func TestAnAmendedRideMarksADeliveredExportStale(t *testing.T) {
	...
	t.Cleanup(func() { _, _ = st.Pool.Exec(context.Background(), "delete from users where id = $1", user.ID) })
	voice := testx.Voice(t, st, testx.Crew(t, st, "Stale", user.ID), "Stale", false)
	...
	saver.AmendRide(ctx, voice, "", "W", workoutJSON, startedAt, hub.RiderRecord{Rider: rider, Samples: samples(80)})
	if staleSince().Valid {
