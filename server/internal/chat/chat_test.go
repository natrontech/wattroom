...
func TestChatRoundTrip(t *testing.T) {
	...
	id1, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "warm-up at 7?", "", time.Now().UnixMilli())
	if !ok || id1 == "" {
		t.Fatal("save 1 failed")
	}
	id2, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(bob.ID), "in", "", time.Now().UnixMilli())
	if !ok || id2 == "" {
	...
	if n, added, ok := svc.toggleReaction(t.Context(), room.ID, room.Slug, id1, store.UUIDString(bob.ID), "🔥"); !ok || n != 1 || !added {
		t.Fatalf("first toggle: %d %v %v", n, added, ok)
	}
	if n, added, ok := svc.toggleReaction(t.Context(), room.ID, room.Slug, id1, store.UUIDString(alice.ID), "🔥"); !ok || n != 2 || !added {
		t.Fatalf("second rider: %d %v %v", n, added, ok)
	}
	if n, added, ok := svc.toggleReaction(t.Context(), room.ID, room.Slug, id1, store.UUIDString(bob.ID), "🔥"); !ok || n != 1 || added {
		t.Fatalf("toggle off: %d %v %v", n, added, ok)
	}
	if _, _, ok := svc.toggleReaction(t.Context(), room.ID, room.Slug, "not-a-uuid", store.UUIDString(bob.ID), "🔥"); ok {
...
func TestReactionRefusedAcrossRooms(t *testing.T) {
	...
	id, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "here", "", time.Now().UnixMilli())
	if !ok {
		t.Fatal("save failed")
	}
	// Toggling it through the OTHER room's slug must refuse — the room is
	// the privacy boundary even for a reaction.
	if _, _, ok := svc.toggleReaction(t.Context(), other.ID, other.Slug, id, store.UUIDString(alice.ID), "🔥"); ok {
...
func TestChatImages(t *testing.T) {
	...
	if _, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "", imgID, time.Now().UnixMilli()); !ok {
...
func TestPruneChatImagesSweepsOnlyUnreferenced(t *testing.T) {
	...
	if _, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "", sent, time.Now().UnixMilli()); !ok {
		t.Fatal("save failed")
	}
...
func TestChatImageFromAnotherRoomIsRefused(t *testing.T) {
	...
	if _, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "look", theirs, time.Now().UnixMilli()); ok {
...
func TestChatLineKeepsItsOwnTimestamp(t *testing.T) {
	...
	if _, ok := svc.saveChat(t.Context(), room.ID, room.Slug, store.UUIDString(alice.ID), "back in ten", "", at); !ok {
		t.Fatal("save failed")
	}
	code, messages := backlog(t, mux, "alice", room.Slug)
