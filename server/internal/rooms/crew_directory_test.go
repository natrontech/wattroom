...
func TestCrewDirectoryListsOnlyWhatAdminsChose(t *testing.T) {
	h := setup(t)
	slug, code := h.createRoom(t, "alice", "Directory Room")
	crew := h.crewOf(t, slug)
	patch := func(who, body string) int {
		status, _ := h.call(t, who, http.MethodPatch, "/api/crews/"+store.UUIDString(crew.ID), body)
		return status
	}
	...
	_, owner := h.call(t, "alice", http.MethodGet, "/api/crews/"+store.UUIDString(crew.ID), "")
	if owner["listed"] != true {
		t.Errorf("the owner's crew page does not say it is listed: %v", owner["listed"])
	}
	_, member := h.call(t, "bob", http.MethodGet, "/api/crews/"+store.UUIDString(crew.ID), "")
...
func TestCrewCardAnswersToTheCode(t *testing.T) {
	h := setup(t)
	slug, code := h.createRoom(t, "alice", "Card Room")
	crew := h.crewOf(t, slug)
