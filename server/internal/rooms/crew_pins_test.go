...
// crewWith is a room, its crew, and bob in it as a plain member.
func crewWith(t *testing.T, h *harness) (slug, crewID string) {
	t.Helper()
	slug, code := h.createRoom(t, "alice", "Velvet Hammer")
	if status, body := h.call(t, "bob", http.MethodPost, "/api/crews/join", fmt.Sprintf(`{"code":%q}`, code)); status != http.StatusOK {
		t.Fatalf("bob joining: %d %v", status, body)
	}
	return slug, h.crewID(t, slug)
}

func (h *harness) crewID(t *testing.T, slug string) string {
	t.Helper()
	return store.UUIDString(h.crewOf(t, slug).ID)
}
