...
func TestTheCrewWearsItsPicture(t *testing.T) {
	h := setup(t)
	crew := h.newCrew(t, "alice", "Crew Picture")
	code := codeOf(crew.Code)
	h.join(t, "bob", crew)
	path := crewPath(crew, "/image")
	...
	// Payloads say so: the crew page, and what a crew settings save answers.
	if _, crewBody := h.call(t, "bob", http.MethodGet, crewPath(crew), ""); crewBody["imageUrl"] != path {
		t.Errorf("the crew payload does not carry the picture: %v", crewBody["imageUrl"])
	}
	if _, saved := h.call(t, "alice", http.MethodPatch, crewPath(crew), `{"name":"Crew Picture"}`); saved["imageUrl"] != path {
		t.Errorf("the settings save's crew does not carry the picture: %v", saved)
	}
	...
	if _, crewBody := h.call(t, "bob", http.MethodGet, crewPath(crew), ""); crewBody["imageUrl"] != nil {
