...
**What worktrees still share:** the Docker infra (`make infra` — one Postgres, one LiveKit) and the dev ports. `make dev-server`/`make dev-web` bind 8080 and 5173 from any worktree, so the second agent to start them gets a bind error, and both write the same database either way. `.claude/launch.json` carries `server-worktree` (:8199) and `web-worktree` (:5199) for exactly this — one spare pair, so a third concurrent agent needs its own. Say what you are about to bind before you bind it.
