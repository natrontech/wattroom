...
- **The VM converges on the newest release by itself.** A systemd timer reads the highest CalVer tag in GHCR, refuses to deploy while riders are on the bike, `pg_dump`s, rolls forward, and retags to the previous release if the new one does not report its own tag and answer `/api/healthz` — then holds there via `WATTROOM_PIN` so it cannot retry a broken release on a loop. Cutting a release *is* deploying it; there is no separate promotion step.
