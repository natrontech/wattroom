...
- No blocking sends/writes while holding a mutex. The hub pattern is: lock → copy what you need → unlock → do I/O on the copy (see `room.run()`, the hub's loop for one voice channel).
- Slow consumers must never stall a voice channel's tick. Timeouts or drop-the-message, never unbounded blocking.
...
- Errors are wrapped with `%w` and carry what the operator needs (channel id, rider id) — but never secrets or tokens.
- Errors either handled or returned, never both (no log-and-return double reporting).

## Security (this app's specifics)

- WS messages are untrusted input: validate bounds (watts 0–3000, seq monotonic) before they touch live state or the DB.
...
- Privacy rules are load-bearing: metrics never leave the session that produced them, no new endpoint exposes another user's data. Flag ANY loosening.
