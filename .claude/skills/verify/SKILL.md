...
starting playback, write all four channels of `wattroom.mixer.v1` in the
...
What you store replaces what was there, and a channel left out comes back at
its default: `board`, the soundboard's own fader, at 0.7 — the rule looks
obeyed and a clip still fires into the voice channel (#990) — and `share`, a
shared screen's own sound, at 1.
The store reads it on load, so do this before the channel page mounts, or reload
...
4. WS/channel changes: open two browser tabs on the same voice channel (`/crew/{id}/v/{channel}`, two riders via `?as=`) and confirm both receive ticks; kill one tab and confirm the server logs the leave and the other tab keeps ticking.
