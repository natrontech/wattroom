...
| Privacy | Live metrics visible **only inside the room, only while riding**. AV is transit-only — **never recorded**. Rides **private by default**, shared per-ride opt-in. Export-all + delete-account (full purge) in MVP. HR is health data; act like it. |
...
- **Visual direction: full synthwave pain cave** ([ADR-0004](docs/decisions/0004-synthwave-visual-identity.md) supersedes the original near-black/watt-yellow palette). Violet-black surfaces (#0a0118 / #1a0736); **two accent hues with distinct jobs** — `--color-neon` (violet #8b2bff) is structural chrome and never glows, `--color-watt` (magenta #ff3d8b) is live data and is the only thing that does. Interval graphs glow like a night ride, live numbers get subtle bloom; sprint moments are the one place the UI is allowed to go loud. Chrome (buttons, forms, editor, settings) stays flat and quiet — the glow means "this is live wattage", never decoration. Medal cards are dark, screenshot-first, built to look good in a group chat.
