...
| Deploy | ~~Kubernetes-native~~ **Superseded by [ADR-0002](docs/decisions/0002-single-vm-compose-deploy.md)**: single VM, docker compose stack (app + Postgres + LiveKit host-network + Caddy TLS). The k8s/LiveKit networking research stays valid in RESEARCH.md §2 for whenever scale demands it. Amended by [ADR-0019](docs/decisions/0019-tagged-releases-and-a-self-converging-vm.md): releases are git tags, the VM converges on a tag pinned in the homelab repo and rolls itself back on a failed health gate. |
| Auth | **Social OAuth only**: Google, GitHub, Strava. No passwords, ever. Strava OAuth doubles as the fitness integration. Amended by [ADR-0029](docs/decisions/0029-one-account-a-set-of-credentials.md): an account holds a **set** of credentials — the providers plus **passkeys**, with a verified email as the recovery attribute and never as the login identifier. "No passwords, ever" still binds (a passkey is not a password); Strava demotes from everyday door to fitness connection. |
| AV | **LiveKit from day one** — voice, camera, screenshare are the product's identity, not a later add-on. |
| Jukebox | **In MVP.** Synced YouTube queue (IFrame Player API per client, server holds queue + position). **Hard TOS constraints (YouTube RMF, verified 2026-04 version)**: player tile ≥200×200 px, always visible while media plays, **nothing overlaid on it** (no HUD over video), no auto-advance while the player is offscreen, and **no audio-only/background mode** — the "music" use case is a visible player tile, period. Spotify: **never API playback** ([ADR-0003](docs/decisions/0003-spotify-via-jam-link-not-api.md) — ducking violates Spotify's no-overlap policy). Amended by [ADR-0015](docs/decisions/0015-self-hosted-music-pool.md): self-hosted MP3 pool joins the same queue; RMF rules apply only while a YouTube entry plays. Amended by [ADR-0018](docs/decisions/0018-one-music-surface-drop-the-jam-card.md): the Jam link-out card is gone — one room, one music surface. Extended by [ADR-0026](docs/decisions/0026-a-playlist-is-one-queue-entry.md): a pasted YouTube playlist is **one** queue entry that walks its own tracks, resolved client-side with no Data API key. |
| Rooms | **Persistent** — a room lives forever, has members and roles (owner, coach, member). Sessions happen *in* rooms. |
| Room UX | ~~**User-switchable layouts**: metrics-first, video-first, media-focus.~~ Plus a **TV mode** (huge numbers, readable at 3 m). Superseded by [ADR-0020](docs/decisions/0020-the-app-takes-discords-shape.md): **one shape that always holds** — a single sidebar (your places; your rooms, the one you are in opened into *its* places; you pinned at the bottom) beside content and the room's people, rendering every route; the three named layouts become three *places* with URLs. TV mode survives unchanged — it is a viewing distance, not a layout preference. |
...
| Integrations | **.fit download** (via **muktihari/fit**) + **Strava auto-upload** in MVP (`POST /uploads`, `activity:write`, async status polling). intervals.icu later. **Strava's June-2026 API terms** ([Agreement](https://www.strava.com/legal/api), [Policy](https://www.strava.com/legal/api_policy)) superseded the Nov-2024 ones this row used to cite. The rule that shaped the design is unchanged: a user's Strava-sourced data may be shown **only to that user** (Policy §2.3, §6.1) — WattRoom only *uploads* its own recorded rides (unaffected) and must never display Strava-pulled data to other room members. Apps still start single-athlete, but **Standard Tier is now a self-upgrade from the API settings dashboard** — 10 athletes, no review queue; beyond 10 is reviewed — and since **2026-06-30 it requires an active Strava subscription on the account that owns the API app**. Three obligations bind anything built on it: raw payloads are a **≤7-day cache** (§6.2), everything goes **≤30 days after deauthorization** (§7.4), and Strava Data may never touch **any AI application** (§5.3) — development, training, evaluation *or operation*, embeddings and RAG included, which binds this repo's own agent tooling and not only the product. Full record, including the **2027-06-01 technical deadline**: [RESEARCH §13.5](docs/RESEARCH.md). |
...
| Privacy | Live metrics visible **only inside the room, only while riding**. AV is transit-only — **never recorded**. Rides **private by default**, shared per-ride opt-in. Export-all + delete-account (full purge) in MVP — the export's scope is set by GDPR Art. 15 / revFADP Art. 25 rather than by taste, and stops at what the rider can already see (#696). HR is health data; act like it — live in-room, never in a shared artifact, never scored ([ADR-0008](docs/decisions/0008-heart-rate-retention.md)). What leaves the platform as mail is governed the same way ([ADR-0030](docs/decisions/0030-what-wattroom-emails.md)): an unconditional security alarm, one opt-in ride switch, and a named never-send list. |
...
- Queue entries resolve client-side with no Data API key — the IFrame player and keyless oEmbed ([ADR-0026](docs/decisions/0026-a-playlist-is-one-queue-entry.md)); Data API `videos.list`/`playlistItems.list` is the documented upgrade path if that proves flaky, not shipped behaviour. Non-embeddable entries (label/Vevo content) are caught by the player's own error event at playback and auto-skipped — there is no pre-check before playback; no in-app `search.list` — paste-a-URL is the golden path.
...
- DRM content (Netflix etc.) not supported; Spotify sync blocked by API terms — YouTube covers music + video for MVP. The room's synced playhead is server time: clients estimate their offset from the tick rather than trusting their own wall clock ([ADR-0018](docs/decisions/0018-one-music-surface-drop-the-jam-card.md) sibling fix, #286).
